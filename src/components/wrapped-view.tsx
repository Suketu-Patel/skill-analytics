...
export default function WrappedView({
  ...
  useEffect(() => {
    // Cold start → show skeleton. Refresh → keep old payload visible.
    if (!cost) setLoading(true);
    ...
    Promise.all([
      wrappedPromise,
      // Region steers the metaphor bank. Read once per fetch so a
      // Settings change takes effect on the next Wrapped visit.
      (() => {
  ...
  return (
    <section className="flex flex-col gap-5">
      ...
      {/* Attribution + contributors live in the GlobalFooter now so
       *  they're visible from every tab, not just Wrapped. */}
    </section>
  );
}
